<template>
  <section :id="sectionId" class="content-section">
    <h2 v-mathjax class="section-title" tabindex="0" @blur="hovered = false" @click="handleTitleClick"
      @focus="hovered = true" @mouseenter="hovered = true" @mouseleave="hovered = false">
      {{ title }}
      <img :class="{ 'clicked': indicatorClicked }" :style="{ opacity: (hovered || indicatorClicked) ? 1 : 0 }" alt=""
        aria-hidden="true" class="section-indicator" src="/link.svg">
    </h2>
