<template>
  <div ref="containerRef" class="clickable-image-container" :class="{ 'loading': !imageLoaded && inView }">
    <img v-if="inView" :alt="alt" :aria-label="`Click to view ${alt} in full size`" :src="src" class="clickable-image"
      role="button" tabindex="0" v-bind="$attrs" @click="handleClick" @keydown.enter="handleClick"
      @keydown.space.prevent="handleClick" @load="onImageLoad" @error="onImageError" />
...
.clickable-image-container {
  ...
  min-height: 200px;
  /* Increased minimum height for better estimation */
  width: 100%;
  max-width: 100%;
}
