...
<template>
  <ContentSection title="Fehler melden">
    <form v-if="!submitted" class="report-form" @submit.prevent="submit">
      <label>
    ...
    <div v-else class="report-success">
      <p>
        <a :href="issueUrl" class="github-link" rel="noopener" target="_blank">
          Fehlerbericht auf Github öffnen
        </a>
      </p>
      <p>
        <button class="report-another-btn" type="button" @click="resetForm">
...
.error-message {
  color: color-mix(in srgb, var(--color-error) 80%, #fff 20%);
