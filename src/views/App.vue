...
export default defineComponent({
  ...
  setup() {
    ...
    watch(currentChapter, (newChapter) => {
      if (newChapter) {
        document.title = newChapter.name;
      } else {
        document.title = 'Mathe Zusammenfassung';
      }
    }, { immediate: true });
...
<template>
  <div class="app-layout">
        ...
        <nav class="chapter-navigation">
          <ul>
            <li v-for="chapter in chapters" :key="chapter.path">
              <RouterLink :to="chapter.path" active-class="chapter-link--active" class="chapter-link"
    ...
    <ImageLightbox :image-alt="currentImageAlt" :image-src="currentImageSrc" :is-open="isLightboxOpen"
      @close="closeLightbox" />
      ...
      <div class="search-popup" @click.stop>
        <div class="search-popup-header">
          <div class="search-popup-input-container">
            ...
            <input ref="searchInput" v-model="searchQuery" class="search-popup-input" placeholder="Suchen..."
              type="text" @keydown="handleSearchKeydown" />
          </div>
          <button class="search-popup-close" @click="deactivateSearch">
            <FontAwesomeIcon icon="fa-solid fa-xmark" />
          </button>
        </div>
        <div class="search-popup-content" @mouseleave="clearHoveredResultIndex()">
          <div v-for="(result, idx) in searchResults" :key="result.link"
            :class="{ 'search-result-item--active': hoveredResultIndex === idx ? true : (hoveredResultIndex === -1 && idx === activeResultIndex) }"
            class="search-result-item" @click="handleSearchResultClick(result.link)"
            @mouseenter="setHoveredResultIndex(idx)" @mouseleave="clearHoveredResultIndex()">
...
.chapter-link--active:hover {
  background-color: color-mix(in srgb, var(--color-surface) 90%, #fff 10%);
